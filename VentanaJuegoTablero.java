...
package practica2.utils.ventanas.ventanaJuego;
...
public class VentanaJuegoTablero extends JFrame {
	...
	private ArrayList<CoordTablero> arrastresRaton;  // lista de arrastres pendientes en pares pulsaci�n - suelta
	...
	public VentanaJuegoTablero( int anchuraVent, int alturaVent, int filas, int columnas, boolean casCuadradas ) {
					...
					pAreaJuego.addMouseListener( new MouseAdapter() {
						...
						public void mouseReleased(MouseEvent arg0) {
							...
							if (pulsacionRaton == null) {  // No ha habido pressed
								// Nada que hacer
							} else if (pulsacionRaton.equals( sueltaRaton )) {  // Misma posici�n tablero pulsaci�n y suelta
								// No se controlan clicks (si se controlaran, se har�a aqu�)
							} else {  // Arrastre (distinta posici�n)
		...
		// Calcula la posici�n del tablero correspondiente a una posici�n
		// real de pixels
		...
		// Calcula la posici�n de pixels de panel correspondiente a una posici�n
		// de tablero
		...
		// Calcula el tama�o interno de la ventana y el tablero
		...
		private void calcTamanyo() {
			...
			if (casillasCuadradas) {  // si son cuadradas hay que centrar el tablero en el panel
				if (pAreaJuego.getHeight() > pAreaJuego.getWidth()) {  // M�s alto que ancho
	...
	/** Cierra y finaliza la ventana de juego (no acaba la aplicaci�n). Llamar siempre a este m�todo al final. 
	...
	// 
	/** Indica si la coordenada es v�lida para el tablero especificado
	...
	/** Devuelve el n�mero de pixels de ancho de cada casilla del tablero
	 * @return
	 */
	...
	/** Devuelve el n�mero de pixels de alto de cada casilla del tablero
	 * @return
	 */
	...
	public CoordTablero readInicioDrag() {
		while (arrastresRaton.isEmpty() && isVisible()) {
			// Espera hasta que el rat�n haga alg�n drag
			try {
				Thread.sleep(10);
	 ...
	 * siempre despu�s de haber le�do la casilla inicial con #waitForDrag()
	...
	/** A�ade al tablero un objeto de juego, que se visualizar� 
	...
	/** Sustituye del tablero un objeto de juego por otro (en su misma posici�n).<br>
	 ...
	 * El objeto debe ser != null y estar a�adido al tablero.
	 ...
	 * realizando una animaci�n (de un paso).<br>
	 * El objeto debe ser != null y estar a�adido al tablero.<p>
	...
	public void movePosTablero( ObjetoDeJuego oj, CoordTablero ct ) {
		if (oj!=null) {
			...
			// oj.setLocation( coordToPixs(ct) );  // Lo hace la animaci�n, no se hace aqu�
		}
	}
	
	/** Anima varios objetos de juego haciendo un escalado en disminuci�n hasta desaparecer.<br>
	 * Los objetos deben ser != null y estar a�adidos al tablero.<p>
	 * No se quitan del tablero ni de la ventana despu�s de la animaci�n (debe hacerse aparte llamando despu�s al m�todo de eliminaci�n)
	...
	/** Devuelve la posici�n de tablero del objeto de juego indicado.<br>
	 * El objeto debe ser != null y estar a�adido al tablero.
	 * @param oj	Objeto de juego del que devolver la posici�n
	 * @return	Posici�n de tablero m�s cercana a ese objeto
	 */
	...
	public static void main(String[] args) {
		...
		VentanaJuegoTablero v = new VentanaJuegoTablero( 960, 720, FILAS, COLS, true );
